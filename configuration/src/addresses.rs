...
pub fn get_addresses() -> &'static Addresses {
    ...
    ADDRESSES.get_or_init(|| {
        ...
        let url = base.join("configuration/toml/addresses.toml").expect("Could not parse addresses server endpoint");        // Retry logic: wait for deployer to finish and save addresses
        let max_attempts = 32;
        let mut wait_time = 2;
