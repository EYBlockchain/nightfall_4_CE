...
pub async fn run_tests(
    ...
    // Test if everything works fine when chainreorg is performed

    // Submit a first deposit.
    let url = Url::parse(&settings.nightfall_client.url)
        .unwrap()
        .join("v1/deposit")
        .unwrap();
    ...
    wait_on_chain(&commitment_hashes, &get_settings().nightfall_client.url)
        .await
        .unwrap();
    info!("First deposit is now on-chain");

    // Perform the chain reorg
    ...
    // query the current layer 1 block number
    // let block_number: u64 = anvil_http_client
    //     .get(anvil_url.join("eth_blockNumber").unwrap())
    //     .send()
    //     .await
    ...
    // ark_std::println!("Current layer 1 block number: {}", block_number);
    // anvil_reorg(&http_client, &anvil_url, 30, false, 5)
    //     .await
    //     .unwrap();
    anvil_reorg(&http_client, &anvil_url, 30, true, 5)
        .await
        .unwrap();
    ark_std::println!("Finished Reorg");

    //	Submit a second deposit.
    let url = Url::parse(&settings.nightfall_client.url)
        .unwrap()
        .join("v1/deposit")
        .unwrap();
    ...
    wait_on_chain(&commitment_hashes, &get_settings().nightfall_client.url)
        .await
        .unwrap();
    info!("Second deposit is now on-chain");
