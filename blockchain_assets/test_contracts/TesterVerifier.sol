...
import {TestVK as VerifyingKey} from "./TestVerificationKey.sol";
import {INFVerifier} from "../contracts/proof_verification/INFVerifier.sol";

contract TesterVerifier is INFVerifier {
     ...
     * @param publicInputs - An array of the public inputs
    ...
    function verify(
        ...
        if (requiredPublicInputCount != publicInputs.length) {
        ...
        assembly {
            ...
            // The first 64 bytes of the initial transcript are zero
            {
                ...
                let public_input_size := mul(mload(NUM_PUBLIC_INPUTS), 0x20)
            ...
            }
            {
                ...
                mstore(VANISH_EVAL, vanishing_numerator)
                {
                    ...
                    // We need the number of public inputs to work out how many times to loop
                    let subtract := sub(p, 1)
                    let ptr := 0x1860
                    for {
                        let i := 0
                    } lt(i, publicInputs.length) {
                        i := add(i, 1)
                    } {
                        // compute the lagrange basis polynomial
                        let lagrange_i := addmod(subtract, zeta, p)
                        mstore(
                            add(PUBLIC_INPUTS_HASH_LOCATION, mul(i, 0x20)),
                            lagrange_i
                        )
                        accumulator := mulmod(accumulator, lagrange_i, p)
                        mstore(add(ptr, mul(i, 0x20)), accumulator)
                        subtract := mulmod(subtract, omega, p)
                    }

                    {
                        ...
                        mstore(0x20, 0x20)
                        mstore(0x40, 0x20)
                        mstore(0x60, mulmod(accumulator, lagrange_n_denom, p))
                        mstore(0x80, sub(p, 2))
                        mstore(0xa0, p)
                        if iszero(
                    ...
                    let piLengthSub := sub(publicInputs.length, 1)

                    let lagrange_denom_inv := mulmod(
                        accumulator,
                        mload(add(ptr, mul(piLengthSub, 0x20))),
                        p
                    )
                    accumulator := mulmod(accumulator, lagrange_n_denom, p)

                    let piEval := 0
                    // We need to multiply `subtract` by omega inverse
                    let omega_inv := mload(OMEGA_INVERSE_LOCATION)
                    subtract := mulmod(subtract, omega_inv, p)
                    subtract := mulmod(subtract, sub(p, 1), p)
                    for {
                        let i := 0
                    } lt(i, piLengthSub) {
                        i := add(i, 1)
                        ...
                        // compute the lagrange basis polynomial
                        let term := sub(piLengthSub, i)
                        // Get the ith inverse
                        let lagrange_i_inv := mulmod(
                            accumulator,
                            mload(add(ptr, mul(sub(term, 1), 0x20))),
                            p
                        )
                        accumulator := mulmod(
                            accumulator,
                        ...
                        // For now we store the inverse in the public input hash location
                        mstore(
                            add(PUBLIC_INPUTS_HASH_LOCATION, mul(term, 0x20)),
                            lagrange_i_inv
                        )
                        piEval := addmod(
                            piEval,
                    ...
                    mstore(PI_EVAL, mulmod(vanish_num_div_n, piEval, p))
                    mstore(
                        LAGRANGE_1_EVAL,
                        mulmod(vanish_num_div_n, accumulator, p)
                    )
                    mstore(
                        LAGRANGE_N_EVAL,
                        mulmod(
