...
where
    ...
    N: NightfallContract,
{
    debug!("Handling client operation: {:?}", operation);
    ...
    debug!("Calling client_operation");
    ...
    let tx_receipt = match operation.transport {
        ...
        Transport::OffChain => process_transaction_offchain(&operation_result).await,
    }
    .map_err(|_| reject::custom(FailedClientOperation))?;
    info!("{} transaction submitted", operation.operation_type);
