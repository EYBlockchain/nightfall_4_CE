...
pub fn get_client_proving_key_locally() -> ProvingKey<UnivariateKzgPCS<Bn254>> {
    let client_pk_path = Path::new("./configuration/bin/proving_key");

    let source_file = find_file_with_path(client_pk_path).unwrap();
...
impl RecursiveProver for RollupKeyGenerator {
    ...
    fn get_vk_list() -> Vec<VerifyingKey<Kzg>> {
        let client_vk = get_client_proving_key_locally().vk.clone();
